...
// Pengetahuan Am) — sets are pre-generated by content owners, never assembled
// live by this app (doc #6: no dynamic question-selection algorithm for MVP).
...
export function getPkskExamSetQuestions(
  ...
  const paper = papers.find((p) => p.title.startsWith(PKSK_EXAM_SET_PAPER_PREFIX));
